...
do
	case $action in
		...
		build)
			...
			docker exec ore-main bash -c "cd /root/contracts/aikon-system-contract && make -p build && rm -rf build/* && ./build.sh -e /usr/opt/eosio/1.8.9/ -c /eosio.cdt/v1.6.3/usr/"
			echo "Build ore protocol contract using EOSIO.CDT v1.6.3"
			docker exec ore-main bash -c "cd /root/contracts/aikon-ore-protocol/contracts && make -p build && rm -rf build/* && ./build.sh"
			;;
		deploy)
