...
docker exec -it nodeos cleos --url http://127.0.0.1:8888 --wallet-url http://keosd:8901 set contract vpow.token /root/contracts/vpow-contract/build/vpow.token/ vpow.token.wasm vpow.token.abi -p vpow.token@active
...
docker exec -it nodeos cleos --url http://127.0.0.1:8888 --wallet-url http://keosd:8901 push action vpow.token create "$(cat create.json)" -p vpow.token@active
